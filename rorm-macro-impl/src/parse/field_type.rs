...
pub fn parse_field_type(tokens: TokenStream) -> darling::Result<ParsedFieldType> {
    ...
    let annos = errors
        .handle(FieldTypeAnnotations::from_attributes(&attrs))
        .unwrap_or_default();
    ...
    if let Some(raw_fields) = errors.handle(get_fields_named(fields)) {
        ...
        for field in raw_fields.named {
            ...
            let Some(annos) = errors.handle(FieldTypeFieldAnnotations::from_attributes(&attrs))
            else {
                continue;
            };
...
pub struct ParsedFieldType {
    pub vis: Visibility,
    pub ident: Ident,
    pub annos: FieldTypeAnnotations,
...
pub struct ParsedField {
    ...
    pub ident: Ident,
    pub ty: Type,
    pub annos: FieldTypeFieldAnnotations,
}

#[derive(FromAttributes, Debug, Default)]
#[darling(attributes(rorm), default)]
pub struct FieldTypeFieldAnnotations {
    // TODO: supporting those would required some kind of patches for MCFs
    //       plus the ability to use FieldType compatible types in patches
    //
